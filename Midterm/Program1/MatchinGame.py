...
def reset_game():
    ...
    # If rows*cols is odd, there is no way to split every tile into a matching
    # pair (one tile would always be left over). Pull one extra tile out of the
    # pairing pool and turn it into a permanently "blocked" tile instead, so the
    # remaining (now even) number of tiles can all be paired up safely and the
    # game can still be completed.
    blocked_id = None
    if len(ids) % 2 != 0:
        blocked_id = ids[randint(0, len(ids) - 1)]
    ...
    # The leftover tile (odd-sized board only) is disabled so it can never be
    # clicked, never affects check_match(), and is visually distinct so the
    # player knows it is not part of the puzzle.
    if blocked_id is not None:
        blocked_button = buttons[str(blocked_id)]
        blocked_button.BUTTON_OBJ.configure(
            text="", bg="gray", fg="gray", relief="flat", state="disabled"
        )
